...
plot_PCs <- function(
  ...
  pcs <- prcomp(t(ct_fid[,-c(1)]), center=T, scale=F, rank.=4)
  var_exp <- pcs$sdev^2/sum(pcs$sdev^2)
  
  pc_df <- data.frame(Label=make.names(rownames(pcs$x)), Variance_Explained=var_exp, pcs$x)
  
  file_path <- paste0("figures/", data_type, "/", expr_type, "/CT_fidelity_", pc_x, "_vs_", pc_y, "_scatterplot_", data_type, "_", expr_type, "_", n_cts, "_CTs_", n_distinct(datinfo$Dataset), "_datasets_", n_genes, "_intersection_genes.pdf")
  ...
  plot_title <- paste("Cell Type", paste(sapply(unlist(strsplit(gsub("_", " ", expr_type), " ")), upper_first), collapse=" "), "Fidelity Scree Plot")
  if(prop_scaled){
  ...
  print(
    ggplot(pc_df[1:n_pcs,], aes(x=1:n_pcs, y=Variance_Explained)) +
      ...
      labs(title=plot_title, 
           subtitle=paste(n_cts, "cell types,", n_distinct(datinfo$Dataset), "datasets,", format(nrow(ct_fid), big.mark=","), "genes")) +
      theme_bw() +
  ...
  plot_title <- paste("Cell Type Fidelity")
  if(prop_scaled){
    plot_title <- gsub("Fidelity", "Proportional Fidelity", plot_title)
  }
  
  plot_sub <- paste(n_cts, "cell types from", n_distinct(pc_df$Study), "studies\n", comma(nrow(ct_fid)), "protein coding genes")
