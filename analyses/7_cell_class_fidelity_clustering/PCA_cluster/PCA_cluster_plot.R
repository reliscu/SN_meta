...
plot_PCs <- function(class_fid, datinfo, cell_class, data_type=c("author_data"), expr_type=c("counts", "QC_counts"), pc_x, pc_y, gene_list=NULL, top_n=NULL, class_expr=NULL, prop_scaled=F, neu_subtypes){
  
  if(ncol(class_fid)-1<as.numeric(gsub("[A-Z]", "", pc_y))){
    print("pc_y is > # of columns in dataset")
  } else {
    
    if(!is.null(top_n)){
      if((ncol(class_fid)-1)>as.numeric(top_n)){
        top_n <- NULL
    ...
    if(!is.null(gene_list)){
      class_expr <- class_expr[is.element(class_expr$SYMBOL, gene_list),]
    }
    ...
    pcs <- prcomp(t(class_fid[,-c(1)]), center=T, scale=F, rank.=4)
    var_exp <- pcs$sdev^2/sum(pcs$sdev^2)
    
    pc_df <- data.frame(Label=make.names(rownames(pcs$x)), Variance_Explained=var_exp, pcs$x)
    
   file_path <- paste0("figures/", data_type, "/", expr_type, "/", cell_class, "_fidelity_", pc_x, "_vs_", pc_y, "_scatterplot_", data_type, "_", expr_type, "_", n_cts, "_CTs_", n_distinct(datinfo$Dataset), "_datasets_", n_genes, "_intersection_genes.pdf")
  
  if(prop_scaled){
    file_path <- gsub("fidelity", "proportional_fidelity", file_path)
  }
  ...
  pdf(file=file_path, width=10, height=10)
    ...
    plot_title <- paste(cell_class, "Subtypes\n", paste(sapply(unlist(strsplit(gsub("_", " ", expr_type), " ")), upper_first), collapse=" "), "Fidelity Scree Plot")
    if(prop_scaled){
    ...
    print(
      ggplot(pc_df[1:n_pcs,], aes(x=1:n_pcs, y=Variance_Explained)) +
        ...
        labs(title=plot_title, 
             subtitle=paste(n_cts, "cell types,", n_distinct(datinfo$Dataset), "datasets,", format(nrow(class_fid), big.mark=","), "genes")) +
        theme_bw() +
        theme(
