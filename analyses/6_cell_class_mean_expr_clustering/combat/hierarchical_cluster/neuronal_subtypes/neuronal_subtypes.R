...
neu_subtypes <- fread("", data.table=F)
...
expr_paths <- expr_paths[grepl(paste(c("INH", "EXC", collapse="|")), expr_paths)]
...
for(i in 1:length(expr_paths)){
  ...
  for(clust_method in c("complete", "ward.D2", "average")){
    plot_dendro(class_expr, datinfo, cell_class, data_type, expr_type, combat, which_genes, gene_list, sim_type, prop_metric, clust_method, top_n, neu_subtypes)
  }
