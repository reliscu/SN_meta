...
plot_dendro <- function(class_expr, datinfo, cell_class, data_type=c("author_data"), expr_type=c("counts", "normalized_counts"), combat=F, sim_type, prop_metric=c("rho", "phs"), clust_method, top_n=NULL){
  ...
  file_path <- paste0("figures/", data_type, "/", expr_type, "/", cell_class, "_mean_expr_dendrogram_", data_type, "_", expr_type, "_", toupper(sim_type), "_", toupper(clust_method), "_", n_cts, "_CTs_", n_distinct(datinfo$Dataset), "_datasets_", n_genes, "_intersection_genes.pdf")
...
dendro_template <- function(datinfo, dendro, color_var, color_lab, n_genes, n_cts){
  ...
  if(n_elements==2){
    ...
    color_vals <- brewer.pal(n_elements, "Set2x")
  } else {
